...
3. `answer` 为空 → 无法判对（**现状**分母可能仍累计；**定稿 P0.5** 将改为不进可判客观分母）
...
| 优先级 | 缺口 | 说明 |
|--------|------|------|
| **P0** | **客观题标准答案** | **已大批量导入**；残年/回忆版暂缺仍空（可接受） |
| **P0.5** | **机判小修** | **已开发** |
| **P1** | **主观题 score 接口** | **当前下一刀**：交卷后按需 `/api/agent/score`；练习开/模考关——见边界文档 §0 |
| **P1.5** | **PDF 题干 ↔ MD 答案完整对齐** | **暂缓优化**（分期 §2.4.3）：缺年、题号错位、主观长文、回忆版天花板；不作答主路径阻塞 |
| **P2** | **答案来源治理** | `answer_source=manual\|ai_draft\|parsed`；可与 AI 草拟流水线同做 |
| **P3** | **运营上传后台 / 解析接口** | 分期 **§2.1.2（暂缓）** |
| **P4** | **换完整源 / 定稿校对** | 回忆版天花板 |
| **P5** | **原版/答案 PDF 下载** | 前端「下载」仍是生成卷面 |

定稿节奏与不做项（社区、chat 打正式分、高数公式机判）见 [主观题评分与Agent边界.md](./主观题评分与Agent边界.md) §0。
