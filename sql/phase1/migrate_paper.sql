...
CREATE TABLE IF NOT EXISTS `paper_attempt_answer` (
  ...
  `user_answer`       TEXT         DEFAULT NULL COMMENT '用户答案（仅 answerable；主观可较长）',
  `correct`           TINYINT      DEFAULT NULL COMMENT '是否正确：1/0；主观题空',
  `ai_score`          DECIMAL(6,2) DEFAULT NULL COMMENT 'AI 评分（仅供参考）',
  `ai_feedback`       TEXT         DEFAULT NULL COMMENT 'AI 评语（仅供参考）',
  `created_at`        DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `updated_at`        DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  PRIMARY KEY (`id`),
